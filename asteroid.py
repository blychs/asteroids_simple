...
class Asteroid(CircleShape):
    ...
    def split(self):
        ...
        rand_velocity_1 = self.velocity.rotate(random_angle)
        rand_velocity_2 = self.velocity.rotate(-random_angle)
        new_radius = self.radius - ASTEROID_MIN_RADIUS
        asteroid_1 = Asteroid(self.position[0], self.position[1], new_radius)
        asteroid_1.velocity = rand_velocity_1 * 1.2
        asteroid_2 = Asteroid(self.position[0], self.position[1], new_radius)
        asteroid_2.velocity = rand_velocity_2 * 1.2
